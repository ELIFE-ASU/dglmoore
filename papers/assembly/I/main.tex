...
In the all of the examples in this sections, we use $\B = \{A,B,C\}$ (highlighted in blue) as the particular basis unless and refer to this this Petri net with $\B$ as the assembly space $\A$.
...
\end{center}
In all of these cases, we've highlighted the chosen (and in fact only) basis for the subspace.
...
In subspace \textbf{c.} we can see that the basis has sites which are not in the basis of $\A$, while the other spaces $\textbf{a.}$, $\textbf{b.}$ and $\textbf{d.}$ do not.
Consequently, \textbf{c.} does not tie the assembly of $AAAB$ to the chosen basis of the $\A$.
Assembly subspaces whose basis is a subset of the larger space will be referred to as \textit{rooted}.
...
\end{center}
were we've chosen $\{A,B,C\}$ and $\{W\}$ as the bases for $\A$ and $\C$, respectively.  
...
\begin{theorem}\label{thm:rooted-image}
    ...
    \TODO{This almost certainly isn't true as defined at present. Assembly maps do not, in general, preserve the basis, $S(f)(\B_A)$ and $S(f)(\B_Z)$ are probably not bases. That said, we might not actually need this. It'll depend on how the proof of \Cref{thm:image-lower-bound} go.}
\end{theorem}
\begin{proof}
    \TODO{Complete}
\end{proof}
...
\begin{proof}
    ...
    \red{This may suffer from the fact that assembly maps do not preserve bases. If that's the case, we should rip all mention of bases out for a future work.}
\end{proof}
