...
\newcommand{\Minim}{\mathcal{Min}}
\newcommand{\Maxim}{\mathcal{Max}}
...
\begin{definition}\label{def:site-completion}
    ...
    \begin{itemize}
        \item $T(X^*) = X$
        \item $\displaystyle S(X^*) = \bigcup_{t \in X} \pi(\sigma(t) + \tau(t))$
        \item $\sigma_{X^*} = \sigma_P|_X$ and $\tau_{X^*} = \tau_P|_X$.
    \end{itemize}
\end{definition}

That this the site completion is, in fact, a subnet follows immediately from the definition of a subnet.
...
It is trivially true that $\G_P \ne \emptyset$ since the set of all sites in $P$ is a generating set.
The set of all generating sets of a Petri net is naturally a subset of the powerset of the sites of $P$.
That is, $\G_P \subseteq \powerset(S(P))$.
As such, $\G_P$ inherits a partial order from $\powerset(S(P))$ with which we can seek ``smaller" generating sets.

\begin{definition}\label{def:basis}
    Let $P$ be a Petri net and let $X$ be a set of generators of $P$, $X \in \G_P$.
...
\begin{definition}\label{def:minimal-maximal}
    Let $P$ be a Petri net and let $x$ be a site in $P$.
    We say that $x$ is minimal in $P$ if the only pathway terminating at $x$ is the zero pathway.
    Similarly, $x$ is maximal in $P$ if the only pathway starting at $x$ is the zero pathway.
    The set of all minimal sites in $P$ is denoted $\Min{P}$, and the set of all maximal sites is denoted $\Max{P}$.
\end{definition}

\begin{lemma}\label{lem:minimal-are-basic}
    Let $P$ be a Petri net.
    For any basis $\B$ of $P$, $\Min{P} \subseteq \B$.
    Furthermore, if $\Min{P}$ is a basis, then it is a unique basis for $P$.

    \red{I'm not $100\%$ sure of the last point, but I think it's true. We'll find out when we start writing the proof.}

    \green{This is important for internal use. In all of the cases we have dealt with in practice, e.g. molecular assembly as applied, the minimal elements form a basis and there is no ambiguity in the Nat. Comm. paper recently published.}
\end{lemma}
\begin{proof}
    \TODO{Complete}
\end{proof}
