...
\begin{definition}\label{def:pathway}
    ...
    \smallskip
    Let $X$ be a subset of sites in $P$. We say that $\gamma$ is \textit{from $X$} if $\pi(\sigma(t_1)) \subseteq X$, an will sometimes say that $\gamma$ starts at $X$.
...
\red{The definition above is very close to the definition of a firing sequence. The primary difference is that we have no number-preserving requirements.}
...
\begin{definition}\label{def:site-completion}
    \TODO{Given a set of transitions, construct the set of all sites and add all the edges.}
\end{definition}
...
\red{The set of all generating sets of a Petri net can be ordered by set inclusion}

\begin{definition}\label{def:basis}
    Let $P$ be a Petri net and let $X$ be a set of generators of $P$.
    We say that $X$ is a \textbf{basis} of $P$ if it is minimal in the partially ordered set of generating sets.
    That is, $X$ is a basis if for any other generating set $Y$ of $P$, if $Y \subseteq X$ then $X = Y$.
\end{definition}
