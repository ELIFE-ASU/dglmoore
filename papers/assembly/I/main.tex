...
\begin{definition}\label{def:pathway}
    ...
    \smallskip
    Let $X$ be a subset of sites in $P$. We say that $\gamma$ is \textit{from $X$} if $\pi(\sigma(t_1)) \subseteq X$.

    \smallskip
    Let $y$ be site, and let $Y$ be a subset of sites.
    We say $\gamma$ is a \textit{to $y$} if $y \in \pi(\tau(t_n))$, and \textit{to $Y$} if $Y \subseteq \pi(\tau(t_n))$.
...
\begin{definition}\label{def:generator}
    \TODO{A set of sites from which there is a pathway to every other object}
\end{definition}

\begin{definition}\label{def:basis}
    Let $P$ be a Petri net and $X \subseteq S(P)$.
    We say that $X$ is a basis of $P$ if $\ldots$.
    \TODO{Complete}
\end{definition}
